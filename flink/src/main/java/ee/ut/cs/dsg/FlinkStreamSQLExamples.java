...
import ee.ut.cs.dsg.source.PageViewsSource;
...
public class FlinkStreamSQLExamples {
    ...
    public static void doStreamHopWindowing(StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
        // Stream windowing
        ...
        Table resultHop = tableEnv.sqlQuery("SELECT nation , HOP_START(rowtime, INTERVAL '1' MINUTE, INTERVAL '1' SECOND) as wStart, HOP_END(rowtime, INTERVAL '1' MINUTE, INTERVAL '1' SECOND) as wEnd, COUNT(*) FROM PageViews GROUP BY HOP(rowtime , INTERVAL '1' MINUTE, INTERVAL '1' SECOND) , nation");
        ...
        TupleTypeInfo<Tuple4<String, Timestamp, Timestamp, Long>> tupleTypeHop = new TupleTypeInfo<>(
                Types.STRING(),
                Types.SQL_TIMESTAMP(), Types.SQL_TIMESTAMP(), Types.LONG());

        // Write results from the table back to an append-only stream
        DataStream<Tuple4<String, Timestamp, Timestamp, Long>> queryResultHop = tableEnv.toAppendStream(resultHop, tupleTypeHop);
    ...
    public static void doStreamTumbleWindowing(StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
        // Stream windowing
        // Assume a source of page views that reports the country from which the page view is coming and time stamp of the view
        // In this example, we first register the stream of page views as a table with two columns nation and rowtime, to refer to the timestamp
        DataStream<Tuple2<String, Long>> pageViews = env.addSource(new PageViewsSource(1000)).assignTimestampsAndWatermarks(new WatermarkAssigner());
        ...
        // We apply a hopping (sliding) window over the table
        // It is like an aggregate query where to group by nation column and the hopping window specification of 1 minute window that updates every second
        Table resultTumble = tableEnv.sqlQuery("SELECT nation  TUMBLE_START(rowtime, INTERVAL '1' MINUTE) as wStart, TUMBLE_END(rowtime, INTERVAL '1' MINUTE),  COUNT(*) FROM PageViews GROUP BY TUMBLE( rowtime , INTERVAL '1' MINUTE, INTERVAL '1' SECOND) , nation");
        // Here, we apply a session window on the same original stream (table) of page views. The session timeout is 1 second
        // We use the Session_start function to project the session start time stamp as a column in the results
        ...
        //In CQL R2S (Relation to stream) transformation.
        // In order to do so, we need to let the stream know about the schema of the tuples
        TupleTypeInfo<Tuple2<String, Long>> tupleTypeTumble = new TupleTypeInfo<>(
                Types.STRING(),
                Types.LONG());

        // Write results from the table back to an append-only stream
        DataStream<Tuple2<String, Long>> queryResultTumble = tableEnv.toAppendStream(resultTumble, tupleTypeTumble);
        // Prints a tuple of 2 of <String, Long> which is the country name and the count of views in the respective window instance.
        queryResultTumble.print();


    }

    public static void doStreamSessionWindowing(StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
        // Stream windowing
        // Assume a source of page views that reports the country from which the page view is coming and time stamp of the view
        // In this example, we first register the stream of page views as a table with two columns nation and rowtime, to refer to the timestamp
        DataStream<Tuple2<String, Long>> pageViews = env.addSource(new PageViewsSource(500)).assignTimestampsAndWatermarks(new WatermarkAssigner());
        ...
        // Here, we apply a session window on the same original stream (table) of page views. The session timeout is 1 second
        // We use the Session_start function to project the session start time stamp as a column in the results
        Table resultSession = tableEnv.sqlQuery("SELECT nation , COUNT(*), SESSION_START(rowtime , INTERVAL '1' SECOND), SESSION_END(rowtime , INTERVAL '1' SECOND), SESSION_ROWTIME(rowtime , INTERVAL '1' SECOND)  FROM PageViews GROUP BY SESSION( rowtime , INTERVAL '1' SECOND) , nation");

        // We need to transform the tuples in the tables carrying query results back to streams.
        //In CQL R2S (Relation to stream) transformation.
        // In order to do so, we need to let the stream know about the schema of the tuples

        TupleTypeInfo<Tuple5<String, Long, Long, Long, Long>> tupleTypeSession = new TupleTypeInfo<>(
                Types.STRING(),
                Types.LONG(),
                Types.SQL_TIMESTAMP(),
                Types.SQL_TIMESTAMP(),
        ...
        DataStream<Tuple5<String, Long, Long, Long, Long>> queryResultSession = tableEnv.toAppendStream(resultSession, tupleTypeSession);
    ...
    public static void main(String[] args) throws Exception {
        ...
        //doStreamHopWindowing(env, tableEnv);
        // doStreamTumbleWindowing(env, tableEnv);

        doStreamSessionWindowing(env, tableEnv);
